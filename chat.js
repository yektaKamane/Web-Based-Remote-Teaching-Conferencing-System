...
groupChat = {
    ...
    CreateReport : function (ReportName = ''){     // Export to excel
      var downloadLink;
    ...
    send : function(){
      ...
      if (msg.value.length > 0) {
        var newmsg = { user: this.username , text: msg.value, score: null, isTrue: null, ID: msgID++ };
    ...
    pushMssg : function(newMssg){
      ...
      document.getElementById(`isTrue-${newMssg.ID}`).addEventListener('change', this.checkTrue)
      document.getElementById(`score-${newMssg.ID}`).addEventListener('keyup', this.giveScore)

        
    },

    checkTrue : function() { //         ********          in bayad ezafe she
   ...
   giveScore : function() { //         ********          in  bayad ezafe she
    let id = Number(this.id.slice(this.id.indexOf('-') + 1));
    let hoof = groupChat.mesgs.find(m => m.ID == id);
